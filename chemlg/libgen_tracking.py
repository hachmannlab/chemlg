...
# MERGE CHANGES FROM LIBGEN: IF_ADD, GET_RULES, AND GETTING THOSE RULES FROM CONFIG IN MAIN FUNCTION.
# update documentation for all the functions
...
class building_blocks(object):
    ...
    Class objects are not created for molecules that are generated by the code.
...
def list_to_smiles(indi_list, init_bb_objs):
    ...
    init_bb_objs: list,
        list of the building block class objects for each of the initial building blocks provided.
            ...
            if not isinstance(k, list):
                ...
                if k:
                    if k[0] == 'C':
                        ...
                        for mol_objs in init_bb_objs:
                            if mol_objs.code == parent_list[-1][0][0]:
                                handle_1 = mol_objs.index_list[handle_1 - 1]
                            if mol_objs.code == parent_list[-1][1][0]:
...
def create_link(mol1, mol2, rules, init_bb_objs):
    """
    ...
    ----------
    mol1: list,
        nested list of first molecule
    mol2: class object
        building block class object of second molecule (initial building blocks)
    rules: dict
        dictionary of rules for library generation 
    ...
    -------
    library_two: list 
        list of tuples: (molecule dict objects, nested lists)
    """

    library_two = []
    avl_pos = count_list(mol1)[0]
    for pos in avl_pos:
        for j in range(mol2.spaces):
            chromosome = deepcopy(mol1)
            nested_lookup(chromosome, pos).append(mol2.code)
            for k in range(mol2.spaces):
                if j==k:
                    nested_lookup(chromosome, pos).append(['C'])
                else:    
                    nested_lookup(chromosome, pos).append([])
            mol_combi = list_to_smiles(chromosome, init_bb_objs)
            mol_combi= pybel.readstring("smi", mol_combi)
            can_mol_combi = mol_combi.write("can")
            if if_add(can_mol_combi, rules):
                temp = molecule(can_mol_combi)
                library_two.append((temp, chromosome))
                # lib_lists.append(chromosome)
...
def generator(combi_type, init_mol_list, init_bb_nested, gen_len, rules_dict, output_dir, init_bb_objs):
    """
    ...
    # lib_nested.append(init_bb_nested)
    library, first_gen = [], []
    for i, j in zip(init_mol_list, init_bb_nested):
        first_gen.append((i, j))
    library.append(first_gen)
    for gen in range(gen_len):
        prev_gen = library[gen]
        lib_temp, new_gen = [], []
        
        ## Now individual processors will generate molecules based on the list of molecules in that processor list.
        chunks_list = scipy.array_split(range(len(prev_gen)), mpisize)
        if chunks_list[rank].any():
            for i in chunks_list[rank]:                       
                for mol2 in init_bb_objs:
                    if combi_type.lower() == 'link':
                        new_gen += create_link(prev_gen[i][1],mol2,rules_dict,init_bb_objs)
                        
                    if combi_type.lower() == 'fusion':
                        new_gen += get_fused(prev_gen[i],mol2,rules_dict)
        new_gen = comm.gather(new_gen, root=0)
        if rank == 0:
            new_gen_mol_list = list(chain.from_iterable(new_gen))      # flatten out the list
        ...
        if gen == gen_len-1:
            ...
            for i in chunks_list[rank]:
                mol_ob = pybel.readstring("smi", new_gen_mol_list[i][0]['reverse_smiles'])
                for atom in list(mol_ob.atoms):
                    ## Removing Francium and Radium atoms. It is easy to convert Francium atom to hydrogen than deleting the atom
                    if atom.OBAtom.GetAtomicNum() == 87 or atom.OBAtom.GetAtomicNum() == 88:
                        atom.OBAtom.SetAtomicNum(1)
                new_gen_mol_list[i][0]['reverse_smiles'] = reverse_mol(mol_ob, list(mol_ob.atoms))
        ...
        for l2 in new_gen_mol_list:
            mol_ob = pybel.readstring("smi", l2[0]['reverse_smiles'])
            smiles_dict[int(mol_ob.OBMol.GetMolWt())].append(l2)                 # appending tuple(dicts and lists) of molecule in dictionary
        items = list(smiles_dict.items())
        chunks_list = scipy.array_split(range(len(items)), mpisize)
        for items_ind in chunks_list[rank]:
            mol_wt, mol_tuple = items[items_ind][0], items[items_ind][1]                     # mols --> list of molecules in that mol_wt category
            tmp_list = []
            for i in mol_tuple:
                mol_ob = pybel.readstring("smi", i[0]['reverse_smiles'])
    ...
    for item1 in list1:
        for item2 in list2:
            ...
            if can_mol_combi not in lib_can_nRa:
                ...
                if if_add(mol_combi_new,rules)==True and shd_add==True:
...
def library_generator(config_file='config.dat', building_blocks_file='building_blocks.dat', output_dir='./'):
    ...
    print_l('Number of building blocks provided = '+str(len(initial_mols))+'\n', output_dir)
    print_l('unique SMILES: \n', output_dir)
    print_l(i_smi_list, output_dir)
    print_l('=============================================================================\n', output_dir)
    
    initial_bb_nested = []
    # list of initial building block class objects
    init_bb_objs = [building_blocks(i, 'F'+str(j+1)) for j, i in enumerate(initial_mols)]

    for bbs in init_bb_objs:
        chromosome = []
        chromosome.append(bbs.code)
        for _ in range(bbs.spaces):
            chromosome.append([])
        # initial molecules in nested lists format
        initial_bb_nested.append(chromosome)

    # Generate molecules
    final_list = generator(combi_type=combi_type, 
                            init_mol_list=initial_mols,
                            init_bb_nested=initial_bb_nested, 
                            gen_len=gen_len, 
                            rules_dict=rules_dict, 
                            output_dir=output_dir, 
                            init_bb_objs=init_bb_objs)
    print_l('Total number of molecules generated = '+str(len(final_list))+'\n', output_dir)
    final_mol_list = [i[0] for i in final_list]
    ...
    df_final_list = pd.DataFrame(final_mol_list)
    if rank == 0:
        if not os.path.exists(output_dest):
            os.makedirs(output_dest)
        df_final_list.drop(['smiles', 'can_smiles'], axis=1).to_csv(os.path.join(output_dir+'final_molecular_library.csv'), index=None)
    
    # processing smi or xyz
    if outfile_type == 'smi':
        if rank == 0:
    ...
    # Creating a seperate output file for each molecule. Files are written to folder with specified no. of files per folder.
        ...
        if rank == 0:
            ...
            for i in range(mpisize):
                ...
                list_to_add = final_mol_list[start:end+1]
