...
def test_exceptions():
    ...
    val = if_add( 'CC', {'2': (10, 20), 'bb_final_lib':False}, 'F1')
    assert not val

    val = if_add( 'CC', {'3': (10, 20), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'CCCCCC', {'4': (10, 20), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'C1CCC1', {'5': (10, 20), 'bb_final_lib':False}, 'F1')
    assert not val

    val = if_add( 'C1CCC1', {'6': (10, 20), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'c1ccc1', {'7': (10, 20), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'CCCC', {'8': (40, 50), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'CC=CC', {'9': (40, 50), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'CC#CC', {'10': (40, 50), 'bb_final_lib':False}, 'F1')
    assert not val 

    val = if_add( 'CCOCOCCCO', {'heteroatoms': [('O', 2)], 'bb_final_lib':False}, 'F1')
    assert not val 
    ...
    val = if_add( 'CCCC', {'14': ['c1ccccc1'], 'bb_final_lib':False}, 'F1')
    # assert not val 

    val = if_add( 'CCCC', {'15': ['c1ccccc1'], 'bb_final_lib':False}, 'F1')
    # assert val
